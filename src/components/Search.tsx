      ...
      <div className="p-6">
        ...
        {error && (
          <div className="mb-6 p-4 bg-error-50 dark:bg-error-900/20 border border-error-200 dark:border-error-800 rounded-lg flex gap-3">
            <svg
              className="w-6 h-6 text-error-600 dark:text-error-400 flex-shrink-0"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <span className="text-sm text-error-800 dark:text-error-300 font-medium">
              {error}
            </span>
          </div>
        )}
              ...
              <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                {results.results.map((result, index) => (
                  <div
                    ...
                    className="p-4 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-700 rounded-lg hover:border-primary-300 dark:hover:border-primary-600 hover:shadow-md transition-all"
                  >
                    {/* Result Header */}
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-semibold ${getSimilarityColor(result.similarity)}`}
                        >
                    ...
                    <div className="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 pt-3 border-t border-gray-200 dark:border-gray-600">
                      <span>
                        Chunk #{result.chunk_index} • Uploaded{' '}
                        {new Date(
                          result.document.upload_date
                        ).toLocaleDateString()}
                      </span>
        ...
        {!results && !loading && !error && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 dark:text-gray-400">
              Enter a query to find relevant content in your documents
            </p>
          </div>
        )}
      </div>
