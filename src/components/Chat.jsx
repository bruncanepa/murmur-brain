...
function Chat({ chatId, allDocuments }) {
  ...
  const handleSendMessage = async () => {
    ...
    if (linkedDocs.length === 0) {
      setError('Please link at least one document to this chat before sending messages.');
      return;
    }
    ...
    try {
      const result = await apiService.sendMessage(chatId, userMessage, selectedModel);
