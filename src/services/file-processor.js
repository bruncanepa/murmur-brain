...
class FileProcessor {
  ...
  async processPDF(filePath) {
    try {
      ...
      console.log(`PDF buffer size: ${(dataBuffer.length / 1024 / 1024).toFixed(2)} MB`);
      ...
      for (let startPage = 1; startPage <= totalPages; startPage += batchSize) {
        ...
        const batchOptions = {
          ...
          pagerender: async function(pageData) {
            ...
            const text = (await textContent).items.map(item => item.str).join(' ');
            return text;
          }
        };
      ...
      console.log(`PDF processing complete: ${totalPages} pages, ${allText.length} characters`);
  ...
  async processPDFStreaming(filePath) {
    try {
      console.log(`Processing PDF in streaming mode: ${path.basename(filePath)}`);
      ...
      console.log(`PDF has ${totalPages} pages. Processing and chunking incrementally...`);
      ...
      for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
        ...
        while (accumulatedText.length >= this.chunkSize) {
          ...
          accumulatedText = accumulatedText.substring(consumedLength - this.chunkOverlap);
        }

        // Log progress
        if (pageNum % 10 === 0 || pageNum === totalPages) {
          console.log(`Processed ${pageNum}/${totalPages} pages, created ${chunks.length} chunks so far`);
        }
      ...
      console.log(`PDF streaming complete: ${totalPages} pages, ${totalChars} characters, ${chunks.length} chunks`);
